...
To cover an interval of message weights, the verifier needs one transfer
bound valid throughout an interval of activation probabilities.
For $0\leq a_-\leq a\leq a_+<1$, coefficient positivity gives the
entrywise inequality
\begin{equation}
 \mathbf N_{m,a}(z)
 =(1-a)\mathbf W_m\!\left(\frac{a}{1-a},z\right)
 \leq(1-a_-)\mathbf W_m\!\left(\frac{a_+}{1-a_+},z\right)
 =:\mathbf M_{m;a_-,a_+}(z).
 \label{eq:ec-uniform-transfer}
\end{equation}
All entries are nonnegative, so this inequality is preserved by matrix
powers.  Substituting $\mathbf M_{m;a_-,a_+}(z)^n$ in
\eqref{eq:ec-marker-bound} gives a uniform tail bound.
For a support interval $[u,v]$, take $a_-:=a_u$ and $a_+:=a_v$.
Multiply the tail bound by an upper bound on
$\sum_{r=u}^{v}\binom{k}{r}$ to obtain the interval contribution.

For dense supports, a simpler bound holds for every fixed invertible
convolution.  When $a_r\leq1/2$, each particular expander output has
probability at most $(1-a_r)^n$.  Invertibility gives one preimage for each
output in the Hamming ball, hence
\begin{equation}
 \Pr[\wt(\x\B\C)\leq L]
 \leq\left(\sum_{h=0}^{L}\binom nh\right)(1-a_r)^n.
 \label{eq:ec-bernoulli-dense}
\end{equation}
The right side is nonincreasing in $r$, so its value at the left endpoint
bounds a dense support interval after multiplying by the message count.
...
Theorem~\ref{thm:ec-exact-first-moment}.  Choose fixed thresholds
$\varepsilon,\tau>0$ and $\gamma>1$ as in the proof of
Theorem~\ref{thm:ea-distance}, replacing $I_\delta$ by
$I^{\mathrm w}_{m,\delta}$.  The uniform sparse tail in
\eqref{eq:ec-uniform-sparse-tail} then bounds each weight-$r$ tail by
$K n^{-\gamma r}$ for $r\rho_n\leq\tau$, with $K$ independent of $n,r$.
This makes the sparse contribution summable.  For intermediate weights, $a_r$ is
...
dense marker $z=\delta/(1-\delta)$, its uniform Perron prefactor, and the
bound of $2^k$ messages give exponent
\[
...
The published nonwrapping theorem uses $m=C_m\log_2 n$ with $C_m>1$ and
imposes $C>10.4344$ and $R<0.1278$ at the same distance~\cite[Theorem~3]{EC}.
These are two of its parameter restrictions, not a sufficient condition for
its failure bound to vanish; that bound also depends on $C_m$.
Our fixed-memory wrapped theorem permits every $R<0.7136$, the binary GV
rate at distance $0.05$.
