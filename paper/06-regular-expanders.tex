...
The cost is also nondecreasing for positive integer $r$, since
$\gamma_{r+1}/\gamma_r=e(1+1/r)^{-r}>1$.
...
the triangle inequality bound a regional point probability by
\[
 2^{-\ell}\sum_{j=0}^{\ell}\binom\ell j|1-2j/\ell|^r
 \leq 2^{1-\ell}\sum_{j=0}^{\lfloor\ell/2\rfloor}
       \binom\ell j e^{-2rj/\ell}
 \leq 2^{1-\ell}(1+e^{-2r/\ell})^\ell.
\]
The first inequality pairs $j$ with $\ell-j$ and uses $1-x\leq e^{-x}$
on $0\leq x\leq1$.  Multiply the regional bounds using independence.
\end{proof}
...
\end{equation}
On a support interval $[u,v]$, bound the conditioning cost by
$\gamma_v^{d_L}$.  Spectral monotonicity from
Section~\ref{sec:binary-ea} bounds the remaining tail by
$\sqrt{1+z}\,z^{-L}\lambda(a^{\Reg}_u,z)^n$.
Multiplying by a bound on $\sum_{r=u}^{v}\binom{k}{r}$ gives the
intermediate interval contribution.
...
For an intermediate interval $[u,v]$, the verifier uses the factor
$\gamma_v^{d_L}$ and the uniform transfer
\eqref{eq:ec-uniform-transfer} with
$a_-:=a^{\Reg}_u$ and $a_+:=a^{\Reg}_v$.
The dense bounds for EA and EC decrease with $r$, so their left-endpoint
values bound each dense interval.  In each case, multiply the tail bound
by an upper bound on the number of messages in that interval.
...
sufficient for the intermediate finite profiles.  Let $d=2h+1\geq3$ and define
\begin{align*}
...
Let $d_R=d\geq3$ be odd, let $r_{\mathrm{mid}}=(k-1)/2$, and let
