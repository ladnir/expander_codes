...
or expected transition counts.  Assume that, conditional on the current
state, this weighted transition law is the same at every step and does not
depend on the earlier path.  Fresh independent randomness at each step
ensures this property for the recursive maps below.
If $\e_0$ selects the initial state, then
\begin{equation}
