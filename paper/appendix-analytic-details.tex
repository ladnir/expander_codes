...
Fix $m\geq1$ and $0\leq\Theta<\infty$.  There are constants
...
Its right one-eigenspace is spanned by the indicator vectors of the active
and all-zero closed classes.  All remaining eigenvalues have modulus less
than one.  Let $\mathbf U$ contain those two indicator vectors, and let $\mathbf V$
...
\end{equation}
The constants are uniform for $\theta\in[0,\Theta]$.
The eigenvalues of $\mathbf G_m(\theta)$ are separated by
\[
 \sqrt{(1-c_m-\theta\alpha_m)^2+4c_m}\geq2\sqrt{c_m}>0.
\]
Therefore the two
...
Decrease $a_0$ so that $a_0<1$.  For $0<a\leq a_0$, the nonnegative matrix
...
proof needs.  For a nonnegative matrix and a positive right eigenvector $v$ with
...
\end{equation}
Fix $\varepsilon>0$ and choose $\theta\geq0$ attaining the supremum in
\eqref{eq:ec-effective-rate}.  Then
...
interval.  The rate function is also continuous there: the all-zero
self-loop gives $\varrho_m(a,z)\geq1-a\geq1/2$, so its defining objective
is at most $\delta\ln z+\ln2$.  This tends uniformly to $-\infty$ as
$z\downarrow0$, allowing the optimization to be restricted to a compact
interval of positive markers.
