...
Sparse linear codes appear inside pseudorandom correlation generators
(PCGs), pseudorandom correlation functions (PCFs), and silent cryptographic
protocols.  A PCF lets two parties use short correlated keys to generate
correlated outputs locally, on demand, at a shared input.  These applications
do not begin with a
...
\paragraph{Impact on EA-based correlation functions.}
An important application is the EA-based PCF of Boyle et al.~\cite{EA}.
Its local evaluation uses one distributed comparison-function evaluation
per nonzero expander entry in the requested row.
The comparison primitive gives the parties shares of a value determined
by a hidden threshold.  The single accumulator is essential to this
construction: accumulated sparse noise can be represented by threshold
functions.  Our EA bounds improve the provable degree--distance tradeoff
without replacing that accumulator.  They therefore target a cost paid at
every PCF evaluation, not just the cost of expanding a complete batch.
Section~\ref{sec:pcf-impact} gives the cost connection and distinguishes
the improvement in proved bounds from the original empirical parameter choices.

\paragraph{One accumulation stage.}
EA also has a simple parallel encoding structure: sparse expansion followed
by one parallel prefix sum.  Our improved distance guarantees retain this
structure.  Repeat--accumulate--accumulate (RAA) codes instead compose two
accumulation stages, with a permutation between them~\cite[Remark~3.4]{EA}.
EA therefore avoids a second dependent scan of the full vector.
Both constructions admit logarithmic arithmetic depth; the advantage is
fewer sequential stages, not a different asymptotic depth class.
Section~\ref{sec:ea-depth} states the cost model, including transposed encoding.

\paragraph{Full asymptotic GV distance at logarithmic degree.}
Our main asymptotic result reaches the Gilbert--Varshamov (GV) distance,
the standard volume-based benchmark for random linear codes.
For every desired failure exponent $A>0$, a sufficiently large constant $C$
gives sampling failure at most $n^{-A}$ for all sufficiently large $n$.
For EA over every fixed finite field, expected expander degree $C\ln n$
gives a vanishing gap of at most $1/\ln n+1/n$ below GV.
The same guarantee holds for binary wrapped EC with any fixed memory.
At fixed rate, encoding uses $O(n\log n)$ expected field operations.
Theorem~\ref{thm:gv-log-degree} proves this claim, with $C$ depending on the
field, limiting rate, desired failure exponent, and recursive map.

For negligible sampling failure, increase the expected degree to $(\ln n)^2$.
Corollary~\ref{cor:gv-negligible} retains the same distance target and gives
failure $\exp(-\Omega((\ln n)^2))$, smaller than every inverse polynomial
in $n$.  Its expected encoding cost is $O(n\log^2 n)$ field operations.
The extra logarithmic factor strengthens the failure guarantee; it is not
needed to approach GV.

The full-field EC construction is instead covered by the finite certificates
below.  Those finite-length certificates are distinct from this growing-degree
asymptotic analysis.  The finite-length tables report
explicit expander degrees and convolution memories to expose the concrete
costs.
...
GV value $0.2430$.
For the wrapped EC construction already proposed in~\cite{EC}, our theorem
permits fixed memory $21$, $C>1.111623$, and $R<0.7136$ at distance $0.05$.
The published nonwrapping theorem instead uses memory proportional to
$\log n$ and imposes $C>10.4344$ and $R<0.1278$ among its conditions.
Thus this asymptotic comparison also changes the analyzed EC variant.
Our finite comparison in Section~\ref{sec:binary-ec} holds the wrapped
ensemble fixed.
...
\begin{enumerate}
  ...
  For EA, the improved degree bounds address the local evaluation cost of
  the existing PCF construction.
