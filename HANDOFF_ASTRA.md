...
Independent-review update: see `notes/CORE_RESULTS_AUDIT_ASTRA.md` for proof
repairs, the distinction between fresh verification and cached replay, and
the remaining audit work. That note supersedes readiness claims below.

Constant-degree follow-up: `notes/CONSTANT_DEGREE_PROBE.md` proves an
obstruction to extending the finite certificates to vanishing-failure,
positive-relative-distance asymptotics at fixed degree and memory. It covers
both regular ensembles. This new research note has not been incorporated
into the manuscript; it changes no finite certificate or implementation.
...
The manuscript title is *Expander-Based Codes at the Gilbert--Varshamov
Bound*. Peter Rindal is the sole author. Rebuild the PDF after audit edits;
the original handoff's page count is no longer current.

The headline is now `O(n log n)` expected encoding cost with tunable
polynomial sampling failure. Theorem `thm:gv-log-degree` proves that, for
every `A > 0`, a constant `C` gives failure at most `n^-A` at expected degree
`C ln(n)`. EA over every fixed finite field and binary wrapped EC with fixed
memory attain GV minus at most `1/ln(n)+1/n`. The dense point-mass correction
is a negative power of `n`, hence smaller than this moving entropy gap.
Corollary `cor:gv-negligible` retains the distance target while increasing
degree to `ln(n)^2`, yielding negligible failure `exp(-Omega(ln(n)^2))`
and expected encoding cost `O(n log^2 n)`. Both costs count field operations.
Do not confuse these growing-degree statements with finite certificates or
extend them to fixed-memory full-field EC. Do not revive the rejected
"concrete linear-time" framing.
...
| recursive map | expander | `d_L/d_R` | memory | `k` | `n` | `L` | failure bits |
|---|---|---:|---:|---:|---:|---:|---:|
| accumulator | left regular | `64/-` | 1 | 1048576 | 2097152 | 230718 | 20.0864 |
| wrapped EC | two-sided regular | `18/9` | 4 | 1048635 | 2097270 | 230742 | 21.6746 |
| wrapped EC | two-sided regular | `14/7` | 6 | 1048635 | 2097270 | 230742 | 26.0875 |
| wrapped EC | two-sided regular | `10/5` | 15 | 1048575 | 2097150 | 230729 | 32.4990 |
| wrapped EC | two-sided regular | `6/3` | 79 | 1048575 | 2097150 | 230729 | 21.4222 |
...
| `d_L/d_R` | memory | `k` | `n` | `L` | failure bits |
|---:|---:|---:|---:|---:|---:|
| `28/14` | 3 | 1048572 | 2097144 | 1032062 | 27.6486 |
| `26/13` | 4 | 1048567 | 2097134 | 1032057 | 71.4724 |
| `24/12` | 6 | 1048572 | 2097144 | 1032062 | 39.5003 |
| `22/11` | 12 | 1048575 | 2097150 | 1032064 | 27.2951 |

The field EA comparator has degrees `30/15`, memory one,
`k=1048575`, `n=2097150`, `L=1032064`, and 29.9715 failure bits.
...
| `d_L/d_R` | memory | `k` | `n` | `L` | failure bits |
|---:|---:|---:|---:|---:|---:|
| `24/12` | 5 | 1048572 | 2097144 | 1015826 | 45.031739 |
| `26/13` | 4 | 1048567 | 2097134 | 1015822 | 135.201876 |

Each row certifies that exact finite triple. It is not a statement for every
padded implementation size.
