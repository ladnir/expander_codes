...
For any fixed convolution $\C\in \dist$ it will be difficult to compute the enumerator. However, by considering all such convolutions together it will be possible to exactly count the number of such triples and thereby compute the enumerator for the ensamble.

\subsection{Exact transfer enumerator}
\label{sec:nonwrapping-transfer-enumerator}

The random convolution admits a compact exact enumerator.  This representation
also gives a direct replacement for the Markov-chain concentration argument in
Expand--Convolute.

For an output prefix, let its reduced state be
$\ell\in\{0,\ldots,\sigma\}$.  State $\ell<\sigma$ means that the prefix ends
in exactly $\ell$ zeros, and state $\sigma$ means that its last $\sigma$
outputs are all zero.  The convolution starts in state $\sigma$.  If
$\ell<\sigma$, the feedback state is nonzero.  A uniform tap vector therefore
makes the next output uniform, independently of the input bit.  If
$\ell=\sigma$, the feedback is zero and the next output equals the input bit.

Let $\mathbf T_\sigma(X,Y)$ be the $(\sigma+1)\times(\sigma+1)$ matrix whose rows and
columns use the state order $0,\ldots,\sigma$.  Its nonzero entries are
\begin{align}
 (\mathbf T_\sigma)_{\ell,0}
   &=\frac{1+X}{2}Y,
 &
 (\mathbf T_\sigma)_{\ell,\ell+1}
   &=\frac{1+X}{2}
 &&(0\leq\ell<\sigma),
 \label{eq:nonwrapping-bivariate-active}\\
 (\mathbf T_\sigma)_{\sigma,0}
   &=XY,
 &
 (\mathbf T_\sigma)_{\sigma,\sigma}
   &=1.
 \label{eq:nonwrapping-bivariate-zero}
\end{align}
The exponent of $X$ records the input weight, while the exponent of $Y$ records
the output weight.

\begin{lemma}[Exact nonwrapping convolution enumerator]
\label{lem:nonwrapping-transfer-enumerator}
Let $\mathbf e_\sigma$ be the unit column vector for state $\sigma$, and let
$\mathbf 1$ be the all-one column vector.  Then
\begin{equation}
 \boxed{
 \enum^{\rConv}_{u,h}
   =
 [X^uY^h]\,\mathbf e_\sigma^\mathsf{T}
 \mathbf T_\sigma(X,Y)^n\mathbf 1.
 }
 \label{eq:nonwrapping-transfer-enumerator}
\end{equation}
\end{lemma}

\begin{proof}
Consider one transition from a nonzero feedback state.  For each fixed input
bit, exactly half of the tap vectors produce each output bit.  Averaging over
the tap vector gives the two entries in
\eqref{eq:nonwrapping-bivariate-active}.  From the zero feedback state, the
output equals the input, which gives
\eqref{eq:nonwrapping-bivariate-zero}.  Matrix multiplication concatenates
transitions and multiplies their input and output markers.  The left vector
selects the initial zero state, and the right vector sums over the final state.
Coefficient extraction therefore gives the expected input--output enumerator.
\end{proof}

For the Bernoulli expander, the intermediate coordinates are independent with
parameter $q_r$ from \eqref{eq:bernoulli-expander-parity}.  Substituting this
input distribution into the transfer matrix removes the input marker.  Define
$\mathbf M_{\sigma,q}(Y)$ by
\begin{align}
 (\mathbf M_{\sigma,q})_{\ell,0}
   &=\frac{Y}{2},
 &
 (\mathbf M_{\sigma,q})_{\ell,\ell+1}
   &=\frac{1}{2}
 &&(0\leq\ell<\sigma),
 \label{eq:nonwrapping-message-active}\\
 (\mathbf M_{\sigma,q})_{\sigma,0}
   &=qY,
 &
 (\mathbf M_{\sigma,q})_{\sigma,\sigma}
   &=1-q.
 \label{eq:nonwrapping-message-zero}
\end{align}

\begin{theorem}[Exact Expand--Convolute first moment]
\label{thm:expand-convolute-first-moment}
Sample $\B\gets\dist^{\mathrm{Ber}}_{k,n,p}$ and an independent nonwrapping
...
\begin{equation}
 ...
 \sum_{h=0}^{L}[Y^h]\,
 \mathbf e_\sigma^\mathsf{T}\mathbf M_{\sigma,q_r}(Y)^n\mathbf 1.
 \label{eq:expand-convolute-first-moment}
\end{equation}
Then
\begin{equation}
 \Pr\left[\exists\xv\neq0:\ \hw{\xv\G}\leq L\right]
 \leq \mu_{\mathrm{EC}}(L).
 \label{eq:expand-convolute-failure}
\end{equation}
In particular, \eqref{eq:expand-convolute-failure} upper-bounds the probability
that the generated code has minimum distance at most $L$.
\end{theorem}

\begin{proof}
Fix a message of weight $r$.  Lemma~\ref{lem:bernoulli-expander-enumerator}
shows that its expander output has independent Bernoulli coordinates with
parameter $q_r$.  Equations \eqref{eq:nonwrapping-message-active} and
\eqref{eq:nonwrapping-message-zero} therefore give its exact output-weight
generating function after the convolution.  Sum its low-weight coefficients,
then multiply by the $\binom{k}{r}$ messages of weight $r$.  Markov's inequality
gives \eqref{eq:expand-convolute-failure}.
\end{proof}

For numerical bounds, coefficient extraction can be replaced by a certified
one-variable estimate.  For every $z\in(0,1]$,
\begin{equation}
 \sum_{h=0}^{L}[Y^h]\,
 \mathbf e_\sigma^\mathsf{T}\mathbf M_{\sigma,q}(Y)^n\mathbf 1
 \leq
 z^{-L}\mathbf e_\sigma^\mathsf{T}\mathbf M_{\sigma,q}(z)^n\mathbf 1.
 \label{eq:expand-convolute-chernoff}
\end{equation}
All coefficients are nonnegative, so each term of degree at most $L$ gains a
factor of at least $z^L$ under evaluation at $z$.  Optimizing $z$ gives a fast
upper bound while retaining the exact state transition matrix.

We next give a direct combinatorial derivation.  First consider a representative
sample with $\sigma=4,w=7,h=9$:
\begin{align*}
