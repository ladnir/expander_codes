...
from flint import arb, arb_mat, arb_poly, ctx, fmpz
...
def full_support_term_arb(
    *, prime: int, n: int, cutoff: int, memory: int, message_weight: int,
    ...
    def branch(values: list[str], field: bool) -> arb:
        z, v = map(arb, values)
        if not (z > 0 and z <= 1 and v > 0 and v <= 1):
            raise ValueError("invalid full-support markers")
        if field and not v >= arb(1) / (prime - 1):
...
def validate_parameters(certificate: dict[str, Any]) -> None:
    """Check the field and integer identities used by regional normalization."""
    parameters = certificate["parameters"]

    def integer(value: Any) -> int:
        if isinstance(value, bool) or not isinstance(value, (int, str)):
            raise ValueError("certificate parameters must be exact integers")
        return int(value)

    order = integer(parameters.get("field_order", parameters.get("prime")))
    if "field_order" in parameters and "prime" in parameters:
        if integer(parameters["prime"]) != order:
            raise ValueError("conflicting field orders")
    if order < 2:
        raise ValueError("field order must be a prime power")
    value = fmpz(order)
    is_prime_power = order & (order - 1) == 0
    if not is_prime_power:
        is_prime_power = bool(value.is_prime())
    if not is_prime_power and value.is_perfect_power():
        for exponent in range(2, order.bit_length()):
            base = value.root(exponent)
            if base**exponent == value and base.is_prime():
                is_prime_power = True
                break
    if not is_prime_power:
        raise ValueError("field order must be a prime power")

    k, n = (integer(parameters[name]) for name in ("k", "n"))
    regions = integer(parameters["region_count"])
    if k < 1 or n < 1 or regions < 1 or n % regions:
        raise ValueError("invalid regional dimensions")
    region_length = n // regions
    if k % region_length:
        raise ValueError("right degree must be an integer")
    if not 0 <= integer(parameters["cutoff"]) <= n:
        raise ValueError("cutoff is outside [0,n]")
    if integer(parameters["memory"]) < 1:
        raise ValueError("memory must be positive")
    if integer(parameters["target_bits"]) < 0:
        raise ValueError("target bits must be nonnegative")
    if integer(certificate["verification"]["precision_bits"]) < 32:
        raise ValueError("verification precision must be at least 32 bits")
