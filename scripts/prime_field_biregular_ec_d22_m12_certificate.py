...
def validate_coverage(certificate: dict[str, Any]) -> None:
    ...
    if int(certificate["sparse_support"]["lo"]) != 1:
        raise ValueError("sparse supports must start at one")
    expected_field = 1
    for band in certificate["sparse_support"]["field_bands"]:
        lo, hi = int(band["lo"]), int(band["hi"])
        if lo != expected_field or hi < lo or hi > SPARSE_LIMIT:
            raise ValueError(f"sparse field coverage breaks at r={expected_field}")
        expected_field = hi + 1
    if expected_field != SPARSE_LIMIT + 1:
        raise ValueError("sparse field coverage is incomplete")
...
def verify_certificate(
    ...
    ec_certificate.validate_parameters(certificate)
