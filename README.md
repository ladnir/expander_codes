...
This repository contains the unified project on enumerator bounds for
Expand--Accumulate (EA) and Expand--Convolute (EC) codes.

- [Current paper (PDF)](output/pdf/exact-enumerator-bounds-for-expander-codes.pdf)
- [LaTeX source](paper/main.tex)
- [Reproduction instructions](SUBMISSION.md)

The canonical repository is <https://github.com/ladnir/expander_codes>.
The project was copied from `permute_conv/expander_codes` on 2026-09-09,
including the then-uncommitted paper and proof-audit updates. See
[MIGRATION.md](MIGRATION.md) for provenance and scope.

The paper, *Expander-Based Codes at the Gilbert--Varshamov Bound*, proves
asymptotic GV distance with `O(n log n)` expected field operations at fixed
rate, for EA over every fixed finite field and binary wrapped EC with fixed
memory. For every desired exponent `A > 0`, a sufficiently large constant
`C` gives failure at most `n^-A` at expected degree `C ln(n)`. The gap below
GV is at most `1/ln(n)+1/n`. A stronger corollary uses expected degree
`ln(n)^2` to make failure negligible, `exp(-Omega(ln(n)^2))`, with
`O(n log^2 n)` expected field operations and the same distance target.
Separate finite-length certificates give explicit degree and memory costs;
we do not advertise them as an asymptotically good linear-time family.
